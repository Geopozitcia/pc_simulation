...
main (void)
{
  ...
  if (col < 20 || row < 30)
    {
      fprintf (stderr, "Сделайте окно терминала больше (мин 20х30)");
      return 1;
    }
  rk_myTermSave ();
  sc_memoryInit ();
              ...
              switch (key)
                ...
                case RUN:
                  ...
                  if (sc_memoryLoad (filename))
                    {
                      messageUI ((char *)"Не удалось загрузить данные", RED);
                      break;
                    }
                  else
                    messageUI ((char *)"Файл загружен", GREEN);
                  break;
                case SAVE:
                  printf ("Имя файла для сохранения: ");
                  scanf ("%s", filename);
                  if (sc_memorySave (filename))
                    {
                      messageUI ((char *)"Не удалось сохранить данные: ", RED);
                      break;
                    }
                  else
                    messageUI ((char *)"Данные сохранены", GREEN);
                  break;
                case RESET:
                  raise (SIGUSR1);
                  break;
          ...
          else
            {
              switch (key)
                ...
                case F5:
                  ...
                  break;
                case F6:
                  printf ("Запись в счетчик команд: ");
                  scanf ("%d", &count);
